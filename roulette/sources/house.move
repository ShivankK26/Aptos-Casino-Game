...
module aptcasino::house {
    ...
    friend aptcasino::game;

    // constants

    /// The seed for the house resource account
    const ACCOUNT_SEED: vector<u8> = b"APTCASINO";
    ...
    public fun get_house_address(): address {
        account::create_resource_address(&@aptcasino, ACCOUNT_SEED)
    }
    ...
    fun assert_signer_is_deployer(signer: &signer) {
        assert!(signer::address_of(signer) == @aptcasino, ESignerNotDeployer);
    }
